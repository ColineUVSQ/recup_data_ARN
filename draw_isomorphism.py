...
with open("fichier_affichage_isomorphisme_nouvelle_metrique.txt", 'w') as fichier :
...
#         dico_graphe = mon_depickler.load()
    with open("dico_comp_complet_metrique_toutes_aretes.pickle", 'rb') as fichier_graphe :
        ...
        with open("sim_extensions_toutes_aretes.pickle", 'rb') as fichier_sim : 
            mon_depickler_sim = pickle.Unpickler(fichier_sim)
            dico_sim = mon_depickler_sim.load()
      
        
#     with open("result_graphes_comp_test/graphe_comp_test_couples_possibles_fichier_5J7L_DA_272_2_fichier_5FDU_1A_48_25.pickle", 'rb') as fichier_graphe :
#         mon_depickler = pickle.Unpickler(fichier_graphe)
#         dico_graphe = mon_depickler.load()      
        ...
        for elt in dico_graphe.keys() :
                ...
                if enlever == False :
                        if "_" + str(elt[0]) + "_" + str(elt[1]) + ".png" not in os.listdir("graphes_comparaison_metrique_toutes_aretes") :
    ...
    #                         
                            ...
                            for element in elt :
                                ...
                                if (cle_sim_1, cle_sim_2) in dico_sim.keys() :
                                    fig.suptitle('sim = '+str(round(dico_sim[(cle_sim_1, cle_sim_2)],2)), fontsize=16)
                                else :
                                    fig.suptitle('sim = '+str(round(dico_sim[(cle_sim_2, cle_sim_1)],2)), fontsize=16)
                ...
                #                                 orange_edges.append((u,v))
                                   ...
                                   # node_labels=dict([(u,(d["nt"], d["type"]))for u,d in G.nodes(data=True)])## if d["type"] != None])
                                    ...
                                    plt.axis('off')              
                            #plt.show()
                            if nom+".png" not in os.listdir("graphes_comparaison_metrique_toutes_aretes") :
                                plt.savefig("graphes_comparaison_metrique_toutes_aretes"+nom+".png") # save as png
                            plt.clf()
                            plt.close()
